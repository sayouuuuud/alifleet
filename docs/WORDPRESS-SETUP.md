...
> # 🚨 الدليل ده مكتوب لسيرفر مختلف عن سيرفرك الفعلي — اقرأ ده الأول
>
> الملف ده مبني على تنصيب **مباشر على الـ VPS** (nginx + php-fpm + mariadb +
> certbot). **السيرفر الفعلي مش كده**: WordPress بيشتغل جوه **Docker تحت إدارة
> Coolify**، و`/var/www` على الهوست **فاضي تمامًا**.
>
> | البند في الدليل | الواقع على سيرفرك |
> |---|---|
> | `/var/www/cms.alifleet.com` | مش موجود. المسار `/var/www/html` **جوه كونتينر** `wordpress-yo985p014jyz554zjo2oo6w7` |
> | `nginx` + `certbot` | مش شغالين. **Traefik v3.6** ماسك 80/443، والشهادات من Coolify |
> | `mariadb` محلي | MySQL 8 في كونتينر `mysql-yo985p014jyz554zjo2oo6w7` |
> | `wp` من أي مكان | `wp-cli` **مش منصّب** — لا على الهوست ولا جوه الكونتينر |
> | `php8.2-fpm` كخدمة | inactive. PHP جوه الكونتينر |
> | سيرفر مخصّص للمشروع | **مشترك** — 16 كونتينر، منهم 6 تطبيقات إنتاج لمشاريع تانية |
>
> **حالة الأقسام:**
>
> | القسم | الحالة |
> |---|---|
> | 1 (تجهيز VPS) · 2 (تنصيب WordPress) | ⚠️ **متنفّذهاش** — معمولة خلاص من Coolify |
> | 3–9 (إضافات · ثوابت · mu-plugin · ACF · داتا · Woo) | ✅ **المنطق صحيح**، بس كل أمر `wp ...` بيتحوّل لـ `sudo wp-agent wp ...`، والنقل بقى خطوتين (`scp` للهوست ثم `wp-agent put`/`stage`). الأوامر الجاهزة في `docs/AGENT.md` |
> | 10 (رفع Next.js) | ⚠️ لسه ساري كمرجع ليك، مش للـ agent |
> | **11 (Nginx + SSL)** | ❌ **ملغي بالكامل** — مش محرَّم بس، **غير قابل للتنفيذ** لأن مفيش nginx شغال. الدومين والشهادة من لوحة Coolify |
> | 12 (تحويل الدومين) | ⚠️ `wp search-replace` بيتنفّذ من جواك، و`WP_HOME`/`siteurl` بيتحكم فيهم متغيرات بيئة Coolify |
> | 13 (اختبارات) · 14 (حل المشاكل) | ✅ سارية، مع تحويل الأوامر زي 3–9 |
>
> **النسخة الاحتياطية:** `wp db export` في القسم الأخير مش شغال. البديل:
> `sudo bash /tmp/agent-user.sh backup` (شوف `docs/AGENT.md`).
...
>
> اللي هنا وبره نطاقك تمامًا: **رفع Next.js (قسم 10)** و **قسم 11 (ملغي أصلًا — مفيش nginx)** و **تحويل الدومين (قسم 12)** و **أي `wp db`** و **أي أمر `docker`**.
>
> وكمان: **كل مسار `/var/www/cms.alifleet.com` في الملف ده غير موجود على السيرفر.**
> لو لقيت نفسك بتكتبه — انت بتتبع الملف الغلط. شغلك كله من `sudo wp-agent`.
...
11. [Nginx + SSL](#11-nginx--ssl) — ❌ **ملغي** (Traefik/Coolify بدالهم)
...
> # ❌ القسم ده **ملغي بالكامل** — غير قابل للتنفيذ على سيرفرك
>
> **مفيش `nginx` شغال على السيرفر** (الخدمة inactive)، ومفيش `certbot`. اللي ماسك
> 80/443 هو **Traefik v3.6** اللي Coolify بيولّد إعداداته من labels على
> الكونتينرات — يعني كل ملف `sites-available` تحت مش هيتقرا من أي حاجة، وتشغيل
> nginx هيتعارض على البورت مع Traefik و**يوقّع كل التطبيقات على السيرفر**، مش
> بس مشروعك.
>
> **الطريق الصح على البيئة دي:**
> 1. ربط الدومين وإصدار شهادة Let's Encrypt = **من لوحة Coolify** على الـ resource
>    بتاع WordPress (تبويب Domains). Coolify بيكتب labels الـ Traefik لوحده.
> 2. الـ headers والـ `client_max_body_size` = من إعدادات الـ resource في Coolify
>    أو `.htaccess` جوه الكونتينر — مش من ملف nginx على الهوست.
>
> **للـ agent:** 🔴 القسم ده محرَّم — ومش محرَّم بس، هو **مستحيل**. ممنوع تشغّل
> nginx، ممنوع `certbot`، ممنوع تلمس Traefik أو Coolify. لو GraphQL مش شغال من
> بره: اختبر عبر Traefik محليًا بـ
> `curl -H 'Host: <الدومين-من-doctor>' http://127.0.0.1/graphql` وبلّغ وبس.
>
> اللي تحت متروك **كمرجع تاريخي للمستخدم لو نقل لسيرفر عادي يوم ما**. متنفّذوش
> على السيرفر الحالي.
