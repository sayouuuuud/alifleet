...
> ## 🔴 للـ agent اللي على السيرفر: الملف ده **خارج نطاقك تمامًا**
>
> ولا سطر من الخطة دي بيتنفّذ على السيرفر. ده شغل **في كود المستودع** (`lib/` و `app/`)، بيتعمل في v0 على `main` وبيتنشر عبر git — مش بـ SSH.
> نطاقك محدّد في **`AGENT-RUNBOOK.md`** وبينتهي عند اختبارات GraphQL (المهمة M7).
> ممنوع تعدّل أي ملف في `app/` أو `lib/` أو `components/`، وممنوع `git commit` أو `git push`.
> **دورك هنا:** إنك تخلّي الشرط اللي فوق يتحقق — وبس.
